...
function Dashboard() {
          ...
          {allResumes.map((resume, index) => {
            ...
            return (
              ...
              >
                ...
                <div onClick={e=> e.stopPropagation()} className="absolute top-2 right-2 hidden group-hover:flex items-center gap-2">
                  <TrashIcon onClick={()=>deleteResume(resume._id)} className="size-5 p-1 hover:bg-white/30 rounded transition-colors" />
                  <PencilIcon onClick={()=>{setEditResumeId(resume._id); setTitle(resume.title)}} className="size-5 p-1 hover:bg-white/30 rounded transition-colors" />
                </div>
              </button>
            );
          })}
  ...
  >
    ...
    >
      <h2 className="text-xl font-bold mb-4 text-white">Upload a Resume</h2>
      ...
      <div>
        <label htmlFor="resume-input" className="text-sm text-slate-400">
          Select resume file
